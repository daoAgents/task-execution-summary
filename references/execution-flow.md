...
> 本节基于第4.1-4.2节的性能基线数据，提出针对性的优化方案。优化目标：标准版报告 P95 延迟从 5 分钟降至 3 分钟。

#### 4.3.1 Step 3 信息收集优化（核心瓶颈，占 40-50%）

**优化方向 A：数据源并行化处理**

当前 Step 3 的三个数据源（对话历史、文件变更、命令日志）采用串行处理。改为并行处理后，总耗时取决于最慢的数据源而非三者之和。

```
当前（串行）：
  对话历史解析 (20s) → 文件变更追踪 (8s) → 命令日志提取 (5s)
  总耗时：33s
...
**预期收益**：减少 30-35% 的 Step 3 耗时

**优化方向 B：增量处理机制**

对于对话追加场景（用户在同一会话中多次请求生成报告），仅处理上次生成后新增的消息，复用已有的 CollectedData。

```
增量处理流程：
  ① 检查是否存在上次生成的 CollectedData 缓存
  ② 存在 → 计算新增消息的 delta
  ③ 仅对 delta 执行信息抽取
  ④ 合并 delta 结果到缓存的 CollectedData
  ⑤ 重新执行质量检查（3.4）
...
**预期收益**：重复生成场景下减少 60-80% 的 Step 3 耗时

**优化方向 C：智能采样策略（超长对话）**

当对话轮数超过 50 轮时，启用关键帧采样策略，避免全量处理导致的延迟激增。

```
采样规则：
  对话轮数 ≤ 50：  全量处理
  对话轮数 51-100：关键帧采样（保留率 70%）
  对话轮数 > 100：  关键帧采样（保留率 50%）
...
**预期收益**：>50轮对话场景减少 40-50% 的 Step 3 耗时，质量损失 < 5%

#### 4.3.2 Step 4 分析处理优化（次要瓶颈，占 35-40%）

**优化方向 A：五维分析并行化**

Step 4 的 5 个分析维度之间无数据依赖，可安全并行执行。

```
当前（串行）：
  目标达成度 (30s) → 时间效能 (25s) → 资源利用 (20s) → 问题模式 (25s) → 协作效果 (15s)
  总耗时：115s
...
**预期收益**：减少 60-70% 的 Step 4 耗时

**优化方向 B：分析结果缓存**

对于相似任务类型的分析，可缓存分析模式和中间计算结果。

```
缓存适用场景：
  - 同类型任务的基准数据（如"软件开发"类任务的典型时间分布）
  - 通用的问题分类模型
  - 标准化的评估量表
...
将常用模板（standard、summary）预编译为高效渲染格式，减少每次生成时的模板解析开销。

```
预编译效果：
  标准版模板：首次渲染 8s → 后续渲染 3s（降低 62%）
  摘要版模板：首次渲染 4s → 后续渲染 1.5s（降低 62%）
```
...
```
流式输出策略：
  ① 元数据和 Frontmatter → 立即输出
  ② 第1章（执行概览） → 优先生成并输出
  ③ 第2-7章 → 按序生成，每章完成后立即推送
  ④ 第8-10章 → 依赖 Step 6 推荐结果，最后输出
  ⑤ 质量评分 → 全部完成后追加

用户感知：
  - 首字节时间从 3-5分钟 缩短为 30-60秒
  - 用户可在等待期间阅读已生成的章节
```
...
| 指标名称 | 计算方式 | 目标值 | 告警阈值 |
|---------|---------|--------|---------|
| P50 延迟 | 50% 分位耗时 | < 3 分钟 | > 4 分钟 |
| P95 延迟 | 95% 分位耗时 | < 5 分钟 | > 8 分钟 |
| P99 延迟 | 99% 分位耗时 | < 10 分钟 | > 15 分钟 |
| 错误率 | 致命错误数 / 总请求数 | < 2% | > 5% |
| 降级率 | 降级生成数 / 总请求数 | < 15% | > 30% |
| 质量评分均值 | 所有报告的 quality_score 均值 | > 85 | < 75 |

**SLA 定义草案**

| 服务等级 | 适用场景 | 可用性 | P95 延迟 | 质量保证 |
|---------|---------|--------|---------|---------|
| 标准版 | 个人用户 | 99.0% | < 5 分钟 | quality_score > 80 |
| 专业版 | 小团队 | 99.5% | < 3 分钟 | quality_score > 85 |
| 企业版 | 大型组织（v3 规划） | 99.9% | < 2 分钟 | quality_score > 90 |
...
| 步骤 | 默认超时 | 最大允许超时 | 超时处理 |
|------|---------|------------|---------|
| Step 1 参数解析 | 5s | 10s | 返回 E001 |
| Step 2 模式识别 | 5s | 10s | 使用默认模式 |
| Step 3 信息收集 | 180s | 300s | 保存已收集数据，发出 E051 |
| Step 4 分析处理 | 180s | 300s | 返回已完成的分析维度 |
| Step 5 报告生成 | 120s | 180s | 回退到简化模板 |
| Step 6 推荐生成 | 60s | 90s | 跳过推荐，使用空 Recommendations |
| Step 7 质量检查 | 30s | 60s | 跳过检查，直接输出 |
| **全流程** | **480s (8min)** | **720s (12min)** | **返回已完成部分** |

**内存使用限制**

| 资源 | 默认限制 | 触发条件 | 降级措施 |
|------|---------|---------|---------|
| 对话历史缓存 | 50MB | 超长对话 (>200轮) | 启用智能采样，丢弃非关键帧 |
| CollectedData | 20MB | 数据量异常大 | 压缩存储，减少冗余字段 |
| AnalysisReport | 10MB | 分析结果过大 | 截断详细数据，保留摘要 |
| DraftReport | 30MB | 详细版超长报告 | 限制单章最大字数 |
| **单次请求总量** | **100MB** | 总内存占用超限 | 中止生成，返回 E041 |

#### 4.3.6 优化路线图

| 阶段 | 优化项 | 预期收益 | 实施难度 | 优先级 |
|------|--------|---------|---------|--------|
| **短期（1-2周）** | Step 4 五维分析并行化 | P95 -30% | 低 | P0 |
| **短期（1-2周）** | 模板预编译 | Step 5 -60% | 低 | P0 |
| **中期（2-4周）** | Step 3 数据源并行化 | P95 -20% | 中 | P1 |
| **中期（2-4周）** | 超时策略实施 | 可靠性 +20% | 中 | P1 |
| **中期（4-6周）** | 增量处理机制 | 重复场景 -70% | 高 | P1 |
| **长期（6-8周）** | 智能采样策略 | 长对话 -40% | 高 | P2 |
| **长期（6-8周）** | 流式输出 | 首字节 -80% | 高 | P2 |
| **长期（8+周）** | 分析结果缓存 | 相似任务 -50% | 高 | P3 |
